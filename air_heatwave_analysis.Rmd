...
heatwave_dur<-temp_appt%>%
  ...
  mutate(Month=month(Date, label=T),
         Year=year(Date),
         duration_95=if_else(duration_95<3, 3, duration_95))

heatwave_dur_year<-heatwave_dur%>%
  group_by(Year, major_city)%>%
  summarise(duration_95_mean=mean(duration_95),
            duration_95_sd=sd(duration_95),
            .groups="drop")%>%
  mutate(duration_95_sd=if_else(is.na(duration_95_sd), 0, duration_95_sd))
...
```{r}
ggplot(heatwave_dur, aes(x=Year, y=duration_95, color=major_city, group=interaction(Year, major_city)))+
...
Aggregated by year
```{r}
ggplot(heatwave_dur_year, aes(x=Year, y=duration_95_mean, ymin=duration_95_mean-duration_95_sd, 
                              ymax=duration_95_mean+duration_95_sd, color=major_city, 
