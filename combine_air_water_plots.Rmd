...
library(ggnewscale)
(map <- ggplot() +
   geom_sf(data = WW_Delta %>% filter(!HNAME%in% c("SAN FRANCISCO BAY" , "SAN PABLO BAY")), color = "gray60", size = 1.5,  fill = "gray80") + 
   geom_sf(data = cities_sf, aes(fill = major_city),  alpha = 0.3 , inherit.aes = FALSE) +
    geom_sf(data = WW_legal_delta, color = "black", fill = NA, size = 2, linetype = "dashed") + 
    geom_sf(data = WW_suisun, color = "black", fill = NA, size = 2, linetype = "dashed") + 
  ...
  new_scale_fill() +
  geom_sf(data = sta_locations_26910, aes(fill = StationName), shape = 21, color = "black", size = 3.5, inherit.aes = FALSE) +
...
air_temp_intensity <- air_temp %>%
  filter(!is.na(heatwave_95_intensity)) %>%
  group_by(major_city) %>%
  mutate(Intensity = mean(heatwave_95_intensity, na.rm = TRUE)) %>%
  ungroup() %>%
  group_by(major_city, Year, heatwave_95, Intensity) %>%
  summarize(meanIntensity = mean(heatwave_95_intensity, na.rm = TRUE),
         sd = sd(heatwave_95_intensity, na.rm = TRUE)) %>%
  ungroup() %>%
  rename(Location = major_city)%>%
  mutate(Medium = "Air")
air_intensity_month <- air_temp %>%
  ...
  mutate(Intensity = mean(heatwave_95_intensity, na.rm = TRUE)) %>%
  ungroup() %>%
  group_by(major_city, Month, heatwave_95, Intensity) %>%
  summarize(meanIntensity = mean(heatwave_95_intensity, na.rm = TRUE),
         sd = sd(heatwave_95_intensity, na.rm = TRUE)) %>%
  ungroup() %>%
  rename(Location = major_city)%>%
...
w_cutoffs = cutoffs %>% left_join(sta_cw)%>%
  mutate(StationName = factor(StationName, levels = station_order))%>%
  mutate(Medium = "Water") %>%
  select(Location = StationName, Month = fMonth, quantile,cutoff, Medium)
...
w_temp_intensity <- w_temp %>%
  ...
  mutate(Intensity = mean(heatwave_95_intensity, na.rm = TRUE)) %>%
  ungroup() %>%
  group_by(StationName, Year, heatwave_95, Intensity) %>%
  summarize(meanIntensity = mean(heatwave_95_intensity, na.rm = TRUE),
         sd = sd(heatwave_95_intensity, na.rm = TRUE)) %>%
  ungroup() %>%
...
w_intensity_month <- w_temp %>%
  ...
  mutate(Intensity = mean(heatwave_95_intensity, na.rm = TRUE)) %>%
  ungroup() %>%
  group_by(StationName, fMonth, heatwave_95, Intensity) %>%
  summarize(meanIntensity = mean(heatwave_95_intensity, na.rm = TRUE),
         sd = sd(heatwave_95_intensity, na.rm = TRUE)) %>%
  ungroup()  %>%
  rename(Location = StationName)%>%
...
cutoffs_95 <- cutoffs %>% filter(quantile == 95)
...
# write_csv(cutoffs_95, "data_clean/cutoffs_95quantile_month_station.csv")
...
# Temp cutoffs
```{r}
p1_air<-ggplot(filter(air_cutoffs, quantile==95), aes(x=Month, y=cutoff, color=Location))+
...
p1_water<- ggplot(filter(w_cutoffs, quantile==95), 
                  aes(x=Month, y=cutoff, color=Location))+
  geom_point(position=position_dodge(width=0.4), alpha=0.8)+
...
png(file=here::here("figs/temperature_cutoffs.png"), height=8, width=10, units = "in", res = 300, pointsize = 14)
p1
dev.off()
```
...
(p2_air<-ggplot(filter(air_heatwave_sum_month, quantile==95), aes(x=Month, y=heatwave_freq, fill=Location))+
...
(p2_water<-ggplot(filter(w_heatwave_sum_month, quantile==95), aes(x=Month, y=heatwave_freq, fill=Location))+
  geom_bar(position=position_dodge(), stat = "identity")+
  scale_y_continuous(expand=expansion(0,0))+
...
(p2<-ggplot(filter(heatwave_sum_month, quantile==95), aes(x=Month, y=heatwave_freq, fill=Location))+
  geom_bar(position=position_dodge(), stat = "identity")+
  scale_y_continuous(expand=expansion(0,0))+
  scale_fill_viridis_d(name="", option = "turbo")+
    facet_wrap(~Medium) + 
...
(p3_air<-ggplot(filter(air_heatwave_sum_year, quantile==95), aes(x=Year, y=heatwave_freq, fill=Location, color=Location))+
...
(p3_water<-ggplot(filter(w_heatwave_sum_year, quantile==95), aes(x=Year, y=heatwave_freq, fill=Location, color=Location))+
    geom_bar(position=position_dodge(), alpha = 0.7, stat = "identity")+
    geom_smooth(method="lm", se=F, show.legend = FALSE, size = 0.6)+
    scale_y_continuous(expand=expansion(0,0))+
...
# Intensity plots
```{r}
(p5_air <- ggplot(filter(air_temp_intensity, heatwave_95==1L), 
...
(p5_water <- ggplot(filter(w_temp_intensity, heatwave_95==1L), 
       aes(x=Year, y=meanIntensity, ymin = meanIntensity - sd, ymax = meanIntensity + sd, 
           color=Location, group=interaction(Year, Location)))+
  geom_pointrange(shape = 17)+
     geom_hline((aes(yintercept = Intensity, color = Location)), linetype = "longdash") +
...
(p6_air <- ggplot(filter(air_intensity_month, heatwave_95==1L), 
...
(p6_water <- ggplot(filter(w_intensity_month, heatwave_95==1L), 
...
int_month_w <- w_temp %>%
  mutate(mean_overall = mean(heatwave_95_intensity, na.rm = TRUE),
         sd_overall = sd(heatwave_95_intensity, na.rm = TRUE)) %>%
