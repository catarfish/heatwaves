...
```{r}
p<-ggplot(filter(cutoffs, quantile==95), aes(x=doy, y=cutoff, color=Station, group=Station))+
  geom_line(alpha=0.8)+
...
p_m<-ggplot(filter(heatwave_sum_month, quantile==95), aes(x=Month, y=heatwave_freq, fill=Station))+
  geom_bar(position=position_dodge(), stat = "identity")+
  scale_y_continuous(expand=expansion(0,0))+
